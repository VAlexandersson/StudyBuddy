...
class ReRankingTask(BaseTask):
 ...
 #   cross_encoder = ReRankerModel()  
    ...
    #self.inference_mediator.rerank_documents(query, documents)
    
    scores = self.inference_mediator.rerank_documents(query, documents)# cross_encoder.predict(query_doc_pairs)
