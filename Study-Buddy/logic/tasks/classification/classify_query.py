...
class ClassifyQueryTask(BaseTask):
  def run(self, context: PipelineContext, logger: Logger) -> PipelineContext:
    ...
    output = self.inference_mediator.classify_query(context.query.text, LABELS, HYPOTHESIS_TEMPLATE) #classifier.classify(context.query.text, LABELS, HYPOTHESIS_TEMPLATE)
    ...
    route = label if (label in ["question", "command"]) else "default"
                            #== "question" or label == "command") else "default"
