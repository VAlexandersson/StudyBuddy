...
def ingest_document(file_path:str, start_page: int=None, end_page: int = None):
    ...
    blocks_df = blocks_df.drop(columns=['id', 'page_id'])
    blocks_df['Summary'] = ''
    blocks_df['Keywords'] = ''
    blocks_df = blocks_df[blocks_df["Chunk"].str.len() >= 30]

    
    return blocks_df.to_dict(orient='records') # list of dicts.
